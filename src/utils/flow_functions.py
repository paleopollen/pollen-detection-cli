    ...
    def filterFlow2UV(self, offsetTensor):  # in pytorch tensor format
        ...
        if kernelSize % 2 == 1:
            ...
            yv, xv = torch.meshgrid(
                [torch.arange(-kernelSize, kernelSize + 1), torch.arange(-kernelSize, kernelSize + 1)])
        else:
            kernelSize = int(offsetTensor.size(0) ** 0.5 / 2)
...
def LKOF_RGB(I1g, I2g, window_size, tau=1e-2):
    ...
    w = int(np.floor(
        window_size / 2))  # window_size is odd, all the pixels with offset in between [-w, w] are inside the window
